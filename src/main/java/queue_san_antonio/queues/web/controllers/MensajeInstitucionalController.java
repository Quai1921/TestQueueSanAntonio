...
public class MensajeInstitucionalController {
    ...
    public ResponseEntity<ApiResponseWrapper<MensajeInstitucionalResponse>> crearMensaje(
        ...
        MensajeInstitucional mensaje = mensajeInstitucionalService.crear(
                configuracionId,
                request.getTipo(),
                request.getTitulo(),
        ...
        // Establecer vigencia si se proporcionó
        if (request.getFechaInicio() != null && request.getFechaFin() != null) {
            ...
            // Recargar mensaje con vigencia actualizada
            mensaje = mensajeInstitucionalService.buscarPorId(mensaje.getId()).orElse(mensaje);
        }

        MensajeInstitucionalResponse response = MensajeInstitucionalMapper.toResponse(mensaje);

        log.info("Mensaje institucional creado exitosamente: {} - ID: {}",
                request.getTitulo(), mensaje.getId());

        return ResponseEntity.ok(
                ApiResponseWrapper.success(response, "Mensaje creado exitosamente")
        );
    }
